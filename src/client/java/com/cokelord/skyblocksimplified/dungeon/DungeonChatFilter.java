...
public final class DungeonChatFilter {
	...
	// Per user request ("Hide sell messages: Hides any message starting with 'You sold'"): item name/count/
	// price all vary per sale, only the fixed leading phrase is checked.
	private static final String SELL_MESSAGE_PREFIX = "You sold";

	// Per user request ("make a chat-declutter feature that removes these messages: 'RARE REWARD! Astraaz
	// found a Recombobulator 3000 in their Obsidian Chest!'. It should be called 'rare reward hider'"): the
	// player name/item/chest tier all vary per drop, only the fixed leading phrase is checked — same
	// convention as SELL_MESSAGE_PREFIX above.
	private static final String RARE_REWARD_PREFIX = "RARE REWARD!";

	// Per user request ("Hide healer orb messages... search for 'You picked up' 'from' 'healing you for' and
	// 'granting you'"): real example quoted: "◕ You picked up an Ability Damage Orb from <player> healing
	// you for 0❤ and granting you +20% Ability Damage for 10 seconds." — every one of these four substrings
	// is fixed regardless of orb type/healer name/heal amount/buff/duration, so requiring all four (rather
	// than the whole line verbatim) covers every real variant.
	private static boolean isHealerOrbMessage(String text) {
		return text.contains("You picked up") && text.contains("from") && text.contains("healing you for") && text.contains("granting you");
	}
