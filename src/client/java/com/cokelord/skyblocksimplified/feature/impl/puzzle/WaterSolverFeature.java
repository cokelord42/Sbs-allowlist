...
public class WaterSolverFeature extends Feature {
	...
	protected void onEnable() {
		...
		if (!listenersRegistered) {
			...
			DungeonBlockDetector.addListener((pos, type) -> {
				...
				// blocks/wool state just needed a moment to finish loading. If the real puzzle's identifying
				// state doesn't actually settle until a lever's ALREADY been pulled once (plausible — the
				// wool "extended" markers this reads are the puzzle's own live progress indicators, not a
				// static room fixture), a fixed post-entry timer gives up long before that ever happens,
			...
			ClientTickEvents.END_CLIENT_TICK.register(client -> {
				...
				// Real bug found (see DungeonBlockDetector listener's own doc comment above): no longer gives
				// up after a fixed retry window — keeps retrying every tick for as long as the player remains
	...
	private void renderInner(GuiGraphicsExtractor graphics) {
		...
		if (showTracer && !candidates.isEmpty()) {
			...
			drawTracerOrEdgeArrow(graphics, firstPos, tracerColor);
			if (candidates.size() > 1 && !candidates.get(1).lever.realPos().equals(first.realPos())) {
				Vec3 secondPos = Vec3.atCenterOf(candidates.get(1).lever.realPos());
				// Real gap found (per user report — same "cant see it with my back turned" complaint as
				// drawTracerOrEdgeArrow's own doc comment, but this second-priority connector line never got
				// that fix): a plain drawLine silently draws nothing at all the moment EITHER endpoint is
				// behind the camera (see WorldRenderUtil#drawLine's own behindCamera() check) — with the solver
				// showing 8 levers at once, losing the lookahead lever's indicator the instant you turn toward
				// the first one was exactly as disorienting as losing the first one entirely used to be. Now
				// falls back to its own independent edge arrow (in secondColor) the same way the first lever
				// already does, and only draws the connecting line when both ends genuinely have somewhere on
				// screen to be drawn.
				if (com.cokelord.skyblocksimplified.highlight.WorldToScreen.project(firstPos).behindCamera()
					|| com.cokelord.skyblocksimplified.highlight.WorldToScreen.project(secondPos).behindCamera()) {
					drawTracerOrEdgeArrow(graphics, secondPos, secondColor);
				} else {
					WorldRenderUtil.drawLine(graphics, firstPos, secondPos, secondColor, 2);
				}
			}
		}
	...
	// Real gap found (per user report — "Try to find a way to render tracers even with back turned. Its
	// very hard to tell the next waterboard lever flick when theres 8 of them at the same time and theres
	// no tracer to the next one"): WorldRenderUtil.drawTracer/drawLine project the target into screen space
	// and simply skip drawing anything at all once that projection reports the point is behind the camera
	// (see WorldRenderUtil#drawLine's own doc comment) — there is genuinely no on-screen pixel to draw a
	// literal line INTO once something is fully behind the player, so this can't be a "make the same line
	// render anyway" fix. Falls back instead to a real screen-edge directional arrow (the same concept a
	// minimap/compass indicator uses) computed from the true horizontal angle between the camera's own
	// facing and the direction to the target, so "which way do I need to turn" is still answered instead of
	// the indicator just vanishing.
	private void drawTracerOrEdgeArrow(GuiGraphicsExtractor graphics, Vec3 target, int color) {
		Minecraft mc = Minecraft.getInstance();
		if (mc.player == null) return;
		if (!com.cokelord.skyblocksimplified.highlight.WorldToScreen.project(target).behindCamera()) {
			WorldRenderUtil.drawTracer(graphics, target, color, 2);
			return;
		}
		Vec3 eye = mc.player.getEyePosition();
		double dx = target.x - eye.x, dz = target.z - eye.z;
		double targetYaw = Math.toDegrees(Math.atan2(-dx, dz));
		double relativeYaw = net.minecraft.util.Mth.wrapDegrees(targetYaw - mc.player.getYRot());
		int width = mc.getWindow().getGuiScaledWidth();
		int height = mc.getWindow().getGuiScaledHeight();
		double angleRad = Math.toRadians(relativeYaw);
		float edgeX = (float) (width / 2.0 + Math.sin(angleRad) * (width * 0.42));
		float edgeY = height / 2f;
		String arrow = Math.abs(relativeYaw) > 150 ? (relativeYaw > 0 ? "§l↻ TURN" : "§l↺ TURN")
			: (relativeYaw > 0 ? "§l→" : "§l←");
		int textWidth = mc.font.width(arrow);
		graphics.text(mc.font, arrow, Math.round(edgeX - textWidth / 2f), Math.round(edgeY), color);
	}
