...
public class DungeonsCopilotFeature extends Feature implements MoveableWidget {
	...
	// Per user request ("Theres still no lever step. This is not for the device, this is for the actual
	// levers that there are two of in every terminals section"): distinct from DEVICE_FINISHED+LEVERS (the
	// shared Levers PUZZLE's own redstone-lamp-grid solved state) — this watches specific real lever BLOCKS
	// (authored the same way BREAKABLE_BLOCKS' own list is, via "Use Looked-At Block") for their POWERED
	...
	// Per user request ("Lightning is not a device type. Remove that."): Lightning has no F7 "completed a
	// device!" broadcast of its own to detect in the first place — it was never a real selectable device.
	public enum DeviceKind { SS, LEVERS, ALIGN, ARROWS }
	...
	// Per user request ("s1 is simon says, s2 is levers device, s3 is align device, s4 is the arrows
	// device") — the real F7 terminal room's 4 progressively-unlocked sections DungeonState already tracks,
	// mapped to this class's own DeviceKind so onChatMessage's DEVICE_FINISHED handling (PD Mode off) can
	// tell which section the generic "completed a device!" line can possibly belong to instead of guessing
	// off who sent it. See that method's own doc comment on why this replaced the old local-player-only gate.
	private static DungeonState.TerminalSection sectionFor(DeviceKind kind) {
		return switch (kind) {
			case SS -> DungeonState.TerminalSection.S1;
			case LEVERS -> DungeonState.TerminalSection.S2;
	...
	// Per user request ("I need a keybind selector for skipping a step in the boss guide. Sometimes i miss a
	// block and cant break it and the boss guide completely breaks. I need to be able to skip. The keybind
	// should not interfere with the dungeon routes and should not say there are keybinds overlapping...
	// because they are in completely different areas and should be able to set as the same bind") — manual
	// override mirroring DungeonRoutesFeature's own nextStepKey pattern exactly. This codebase's own
	// keybind-rebinding UI (MainScreen) has no cross-feature conflict detection at all, so sharing a
	// physical key with that one (or any other keybind) is already safe with zero extra code here.
	private final net.minecraft.client.KeyMapping skipStepKey = new net.minecraft.client.KeyMapping(
		"key.skyblocksimplified.boss_guide_skip_step", com.mojang.blaze3d.platform.InputConstants.Type.KEYSYM,
	...
	public DungeonsCopilotFeature() {
		...
		instance = this;
		net.fabricmc.fabric.api.client.keymapping.v1.KeyMappingHelper.registerKeyMapping(skipStepKey);
	...
	@Override
	public java.util.List<net.minecraft.client.KeyMapping> getKeybinds() { return java.util.List.of(skipStepKey); }
	...
	private void onChatMessage(String text) {
		...
		List<CopilotItem> myItems = new ArrayList<>(activeClassItems());
		// Real bug found (per user report — "if you have a PD mode off levers device a lot further in [...]
		// it bugs out and skips to that step when doing predev"): DEVICE_FINISHED/LEAP_USED/TERMINAL_DONE all
		// key off ONE shared, generic chat line each (Hypixel's own "completed a device!" line in particular
		// never says WHICH device — SS, Levers, Align, and Arrows all send the exact same text). The loop
		// below used to keep going after advancing currentStep once, still holding the SAME cached
		// isLocalDeviceCompleted/etc. boolean from this one incoming line — so a single "completed a device!"
		// line (e.g. from an early SS device finishing) satisfied every LATER non-PD DEVICE_FINISHED item too
		// (a Levers step meant for much further into the guide, well into the real terminals phase), jumping
		// currentStep all the way past it immediately. Sorted into ascending step order and stops at the
		// FIRST of these shared-signal steps it advances past in a single pass — a single real event should
		...
		for (CopilotItem item : myItems) {
			...
			if (item.type == StepType.DEVICE_FINISHED) {
				...
				// completes the levers device"): this used to also require the message's own sender name to
				// equal the local player's — since Hypixel's "completed a device!" line never says which
				// device, gating on "was it ME" was really an attempt to guess which device just finished, and
				// it silently ate every OTHER real player's completion (like an early-enter teammate). Gated on
			...
			if (item.type == StepType.LEAP_USED) {
				...
				if (isLocalLeapCompleted) { currentStep = nextStepAfter(myItems, item.stepNumber); return; }
				continue;
			}
			if (item.type == StepType.TERMINAL_DONE) {
				// Real bug found (per user report — "Completing the levers device still skips a LOT of
				// steps... If pd mode is off it should ONLY search for the chat message and nothing else"):
				// onTick's own live Levers lamp-grid read used to run for EVERY TERMINAL_DONE item
				// unconditionally, with no per-item opt-in — so any predev/practice "all lamps light up at
				// once" fake-completion (a known real predev artifact, distinct from a genuine per-terminal
				// chat line) satisfied every TERMINAL_DONE step from currentStep onward in one tick, not just
				// one meant for Levers specifically. Reusing item.pdMode (same field/UI toggle DEVICE_FINISHED
				// already exposes for ALIGN/LEVERS) to make the two signals mutually exclusive per item, same
				...
				if (isLocalTerminalActivated) { currentStep = nextStepAfter(myItems, item.stepNumber); return; }
				continue;
			}
	...
	public void onTick(Minecraft client) {
		...
		// Per user request ("I need a keybind selector for skipping a step in the boss guide... the boss
		// guide completely breaks. I need to be able to skip"): manual override, checked before the
		// justEnteredBoss short-circuit below so a skip still works even on the very tick a fresh step was
		// just reset to. Reuses the same nextStepAfter() every other completion path already advances
		// through, so it lands on the next step that actually has items defined, same as normal progression.
		while (skipStepKey.consumeClick()) {
		...
		// Real bug found (per user report — see onChatMessage's own doc comment on this same item.pdMode
		// check): this used to run for EVERY TERMINAL_DONE item unconditionally, not just ones actually meant
		// to represent Levers — now gated on item.pdMode (mutually exclusive with the chat check above), same
		// "re-checks the step you're currently ON, advances PAST it" shape as that loop above.
		for (CopilotItem item : myItems) {
			if (item.stepNumber < currentStep) continue;
			if (item.type != StepType.TERMINAL_DONE || !item.pdMode) continue;
		...
		for (CopilotItem item : myItems) {
			...
			boolean done = switch (item.type) {
				...
				case LEVER_FLIPPED -> !item.blocks.isEmpty() && isLeverFlippedSatisfied(item, client.level);
				case TITLE -> false;
	...
	/** True once every tracked lever (authored via "Use Looked-At Block", same as BREAKABLE_BLOCKS/
	 *  BLOCK_WATCH's own `blocks` list) has actually been flipped ON — unlike BLOCK_WATCH, which only ever
	 *  compares a block's TYPE name and so can never tell a flipped lever from an unflipped one (a lever's
	 *  block type never changes, only its POWERED blockstate property does, the same real property
	 *  isLeversDeviceComplete already reads for redstone lamps). Requires the block to actually BE a real
	 *  vanilla Lever, not just anything reporting powered=true, so an author who points this at the wrong
	 *  block gets an honestly-never-satisfied step instead of a false positive on some unrelated redstone
	 *  component. */
	private static boolean isLeverFlippedSatisfied(CopilotItem item, Level level) {
		for (BlockPos pos : item.blocks) {
			if (!level.isLoaded(pos)) return false;
			var state = level.getBlockState(pos);
			boolean isFlippedLever = state.is(net.minecraft.world.level.block.Blocks.LEVER)
				&& state.getValue(net.minecraft.world.level.block.state.properties.BlockStateProperties.POWERED);
			if (!isFlippedLever) return false;
		}
		return true;
	}
	...
	private static void renderWorldMarkers() {
		...
		try {
			for (CopilotItem item : instance.currentStepItems()) {
				switch (item.type) {
					case BREAKABLE_BLOCKS, BLOCK_WATCH, LEVER_FLIPPED -> {
