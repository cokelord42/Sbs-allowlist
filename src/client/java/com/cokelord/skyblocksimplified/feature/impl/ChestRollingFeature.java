...
public class ChestRollingFeature extends Feature {
	...
	// either spelling now; captured group 1 is only ever used for the bedrock-only check and as a label in
	// chestContentsKey, neither of which cares which spelling matched.
	...
	// Per user request ("make sure the animation only plays once per chest, I don't want it playing every
	// time I open the chest"): containerId alone can't tell "the same real chest, reopened" from "a genuinely
	// new chest" — the server assigns a fresh containerId to every screen open, including re-viewing a chest
	// you already looked at (e.g. clicking the same Croesus entry again). Keyed on the chest's own actual
	// contents instead (chest type + every non-empty slot's item id/count, concatenated) — stable across
	// re-opens of the literal same chest, distinct for any two chests that don't happen to contain the exact
	// same set of items. Per user report ("the mod forgets which chests you have opened when you restart"):
	// this used to be session-only on purpose (see the removed comment this replaced) — now persisted, since
	// a client restart mid-run (or revisiting an already-animated Croesus chest in a later session) replayed
	// the whole roll animation again for a chest that had already shown it. A LinkedHashSet in insertion
	// order so PLAYED_KEYS_CAP can evict the oldest entries first once it's full, instead of growing the
	// saved config file forever across months of real play.
	private final java.util.Set<String> playedChestKeys = new java.util.LinkedHashSet<>();
	private static final int PLAYED_KEYS_CAP = 2000;
	...
	public ChestRollingFeature() {
		...
		net.fabricmc.fabric.api.client.item.v1.ItemTooltipCallback.EVENT.register((stack, context, flag, lines) -> {
			...
			String tier = chestTierName(stack);
			if (tier == null || isOpenedChestOption(stack)) return;
	...
	protected void onEnable() {
		...
		ContainerClickRegistry.setRule("chest_rolling", (slot, slotId, mouseButton, type) -> {
			if (!isRollActive()) return false;
			Minecraft mc = Minecraft.getInstance();
			return mc.gui.screen() instanceof AbstractContainerScreen<?> screen
				&& screen.getMenu().containerId == lastProcessedContainerId;
		});
	}
	...
	private void tryStartRoll(String chestTypeName, AbstractContainerScreen<?> screen) {
		...
		String chestKey = chestContentsKey(chestTypeName, screen);
		if (!playedChestKeys.add(chestKey)) return;
		if (playedChestKeys.size() > PLAYED_KEYS_CAP) {
			playedChestKeys.remove(playedChestKeys.iterator().next());
		}
	...
	private static String chestContentsKey(String chestTypeName, AbstractContainerScreen<?> screen) {
		var playerInventory = Minecraft.getInstance().player != null ? Minecraft.getInstance().player.getInventory() : null;
		StringBuilder key = new StringBuilder(chestTypeName);
		for (Slot slot : screen.getMenu().slots) {
			if (playerInventory != null && slot.container == playerInventory) continue;
			ItemStack stack = slot.getItem();
			if (stack.isEmpty()) continue;
			String id = SkyblockNbtUtils.getItemId(stack);
			key.append('|').append(id != null ? id : stack.getItem()).append('x').append(stack.getCount());
		}
		return key.toString();
	}
	...
	// isRollActive() (both phases combined) still gates click-blocking — same reasoning as before, just now
	// covering settle too so the player can't snatch the reveal away before they see it.
	...
	public static boolean shouldReplaceRender(AbstractContainerScreen<?> screen) {
		return instance != null && instance.isSpinning() && screen.getMenu().containerId == instance.lastProcessedContainerId;
	}

	public static void renderReplacement(GuiGraphicsExtractor graphics, AbstractContainerScreen<?> screen) {
		if (instance != null) instance.renderRoll(graphics, screen);
	}
	...
	public JsonElement savePersistedData() {
		...
		com.google.gson.JsonArray playedArray = new com.google.gson.JsonArray();
		for (String key : playedChestKeys) playedArray.add(key);
		obj.add("playedChestKeys", playedArray);
		return obj;
	}

	/** Real bug found (per user report — "Config is not optimized, its still 360000 characters when
	 *  exporting the mod config to clipboard"): {@code playedChestKeys} is a local "don't replay the roll
	 *  animation for a chest I've already seen" memory cache, capped at {@link #PLAYED_KEYS_CAP} (2000)
	 *  entries — each one a full chest-type-plus-every-item-id-and-count concatenation (see
	 *  chestContentsKey's own doc comment). A long-played account with hundreds/thousands of real distinct
	 *  chests seen easily reaches megabytes of raw JSON before gzip even runs, and unlike settings JSON
	 *  (short, highly repetitive key names) this is closer to high-entropy data gzip can't shrink much —
	 *  almost certainly the actual dominant contributor to the reported export size. It's also not a real
	 *  "setting" in any meaningful sense — nobody wants to carry their exact played-chest history into an
	 *  export on another device — so it's the one thing trimmed here; every other field stays identical to
	 *  {@link #savePersistedData()}. */
	@Override
	...
	public void loadPersistedData(JsonElement data) {
		...
		if (obj.has("playedChestKeys")) {
			playedChestKeys.clear();
			for (JsonElement el : obj.getAsJsonArray("playedChestKeys")) playedChestKeys.add(el.getAsString());
		}
	}
