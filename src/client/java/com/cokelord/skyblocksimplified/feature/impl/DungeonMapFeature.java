...
public class DungeonMapFeature extends Feature implements MoveableWidget {
	...
	public enum RoomLabelMode { SECRETS, NAME, CHECKMARK, NAME_AND_SECRETS }
	...
	// Per user request ("Add a slider for increasing head size"): a plain multiplier on top of each head
	// marker's already-existing size formula (which itself already varies by widget size/GRID cell), rather
	// than a new fixed-pixel size — keeps heads proportional to the map at every render scale/zoom level.
	private float headSizeMultiplier = 1.0f;
	// Per user request ("Add a toggle for making the head outline the class color"): drawRotatedHead's own
	// 2px border was always a fixed black — this makes it optionally use the same per-class DungeonClass#color
	// already used elsewhere on this map (see drawTeammateMarker's own class-suffix text) instead, falling
	// back to black for any teammate with no known class yet.
	private boolean headOutlineClassColor = false;
	...
	// Per user request: only meaningful while showNamesMode isn't OFF — see drawTeammateMarker's own doc
	// comment on why the class is drawn as a separate bold, class-colored text run after the name.
	private boolean showClassSuffix = false;
	...
	private static boolean loggedEmptyMarkerDump = false;
	...
	public Size render(GuiGraphicsExtractor graphics, int x, int y, float scale) {
		...
		// Per user request ("make the map turn into the map in the player inventory when the run is over,
		// so it shows the score"): Hypixel's own real vanilla map item visually swaps to a score stamp once
		// a run completes — SplitsFeature.isRunFinished() (true from the run's own last split firing, until
		// the next run starts) is this codebase's existing, already-reliable "the run is objectively over"
		// signal, so this reuses it directly rather than inventing a second one.
		if (com.cokelord.skyblocksimplified.feature.impl.SplitsFeature.isRunFinished()) {
			drawScorePanel(graphics, x, y, size);
			int infoHeightFinished = mapInfoEnabled ? drawMapInfo(graphics, x, y + size + 2, scale) + 2 : 0;
			return new Size(size, size + infoHeightFinished);
		}
	...
	/** Replaces the normal room/door grid with a big centered score readout once the run is over, same
	 *  square footprint as the normal map so the widget's own edit-screen hitbox doesn't jump around —
	 *  reuses this widget's own background/outline settings so it still matches whatever theme the user
	 *  already picked for the map itself. No-ops (draws nothing) if the Dungeon Score Calculator module
	 *  isn't around to compute a score from. */
	private void drawScorePanel(GuiGraphicsExtractor graphics, int x, int y, int size) {
		if (size <= 0) return;
		if (backgroundEnabled) {
			int alpha = Math.round(Math.max(0f, Math.min(1f, backgroundOpacity)) * 255f);
			graphics.fill(x, y, x + size, y + size, (alpha << 24) | (backgroundColor & 0xFFFFFF));
		}
		DungeonScoreCalculatorFeature calculator = DungeonScoreCalculatorFeature.getInstance();
		if (calculator != null) {
			DungeonScoreCalculatorFeature.ScoreBreakdown score = calculator.computeScore();
			net.minecraft.client.gui.Font font = Minecraft.getInstance().font;
			String tierColor = DungeonScoreCalculatorFeature.tierColorCode(score.tier());
			String tierLine = tierColor + "§l" + score.tier();
			String scoreLine = "§7" + score.total() + " pts";
			// Scaled up well past the map's own normal text sizes — this is meant to read like the real
			// vanilla map item's own big score stamp, not another small label.
			float tierScale = size / 48f;
			int tierWidth = Math.round(font.width(tierLine) * tierScale);
			int tierHeight = Math.round(font.lineHeight * tierScale);
			int tierX = x + (size - tierWidth) / 2;
			int tierY = y + size / 2 - tierHeight;
			graphics.pose().pushMatrix();
			graphics.pose().translate(tierX, tierY);
			graphics.pose().scale(tierScale);
			graphics.pose().translate(-tierX, -tierY);
			graphics.text(font, tierLine, tierX, tierY, 0xFFFFFFFF);
			graphics.pose().popMatrix();
			int scoreWidth = font.width(scoreLine);
			graphics.text(font, scoreLine, x + (size - scoreWidth) / 2, y + size / 2 + tierHeight / 2, 0xFFFFFFFF);
		}
	...
	private static void renderOffMapMarkers(GuiGraphicsExtractor graphics, int x, int y, int size, boolean heads, java.util.List<MarkerHit> outHits) {
		...
		int markerSize = Math.round(Math.max(14, Math.round(size * 0.09f)) * headSizeMultiplier(instance));
		...
		for (int i = 0; i < missing.size(); i++) {
			...
			drawPlayerMarker(graphics, acp.getSkin(), px, markerY, markerSize, heads, player.getRenderYaw(), headOutlineColor(player));
	...
	private static void drawBossMap(GuiGraphicsExtractor graphics, int x, int y, int size, BossMapEntry entry, BossPhase phase,
		...
		try {
			...
			// 0.35x/5px min cell size. headSizeMultiplier applies on top (see field's own doc comment).
			float markerSize = Math.max(5f, (size / (float) GRID) * 0.35f) * headSizeMultiplier(instance);
			for (DungeonPlayer player : DungeonState.getTeammates()) {
				...
				drawPlayerMarker(graphics, acp.getSkin(), px, py, Math.round(markerSize), heads, player.getRenderYaw(), headOutlineColor(player));
				if (showNames) {
	...
	private static void drawMap(GuiGraphicsExtractor graphics, int x, int y, int size, RoomLabelMode labelMode,
		...
		for (DungeonRoom room : DungeonScan.rooms) {
			...
			// Per user request ("add an option to show room name AND secret count, room name with secret
			// count displayed underneath it"): unlike every other label mode, this one needs a SECOND line
			// rather than swapping which single line shows — reuses the exact same "0/0"/fully-cleared
			// suppression rules SECRETS mode already applies (see roomLabel's own SECRETS case) so the second
			// line only appears when there's a real, still-meaningful count to show.
			String text = knownButUnexplored ? roomTypeLabel(room.type) : roomLabel(room, labelMode);
			String secondLine = null;
			if (!knownButUnexplored && labelMode == RoomLabelMode.NAME_AND_SECRETS && room.data != null) {
				int total = room.data.getSecrets();
				if (total > 0 && room.checkmark != MapCheckmark.GREEN) {
					int found = room.secretsFound;
					secondLine = (found >= 0 ? found : "?") + "/" + total;
				}
			}
			...
			if (secondLine != null) {
				drawStackedCenteredText(graphics, mc, text, secondLine, x + (float) (avgX * cell), y + (float) (avgZ * cell), cell, labelColor);
			} else {
	...
	private static void drawTeammateMarker(GuiGraphicsExtractor graphics, Minecraft mc, DungeonPlayer player,
		...
		// Per user request ("Make the player heads on the map smaller"): reduced from 0.5x/6px min to
		// 0.35x/5px min cell size. headSizeMultiplier applies on top (see field's own doc comment).
		int markerSize = Math.round(Math.max(5f, cell * 0.35f) * headSizeMultiplier(instance));
		drawPlayerMarker(graphics, skin, px, py, markerSize, heads && skin != null, yawDeg, headOutlineColor(player));
		if (showNames) {
			String name = player.name;
			// Per user request ("add a new option in the dungeon map when names on map is on... that shows
			// class in bold and color after their name"): the class's own real DungeonClass#color (the same
			// per-class color used everywhere else in this codebase) rather than a fixed color, drawn as a
			// separate bold text run right after the name — a single graphics.text call can't mix two colors
			// within one string without embedding legacy codes, and DungeonClass only exposes a raw ARGB int,
			// not a §-code, so two calls is simpler than reverse-mapping that int to the nearest legacy code.
			String classSuffix = instance != null && instance.showClassSuffix && player.clazz != null && player.clazz != DungeonClass.EMPTY
				? " " + classAbbreviation(player.clazz) : "";
			int nameWidth = mc.font.width(name);
			int classWidth = classSuffix.isEmpty() ? 0 : mc.font.width("§l" + classSuffix);
			int totalWidth = nameWidth + classWidth;
			int textX = Math.round(px - totalWidth / 2f);
			int textY = Math.round(py - markerSize / 2f - 9);
			graphics.text(mc.font, name, textX, textY, 0xFFFFFFFF);
			if (!classSuffix.isEmpty()) {
				graphics.text(mc.font, "§l" + classSuffix, textX + nameWidth, textY, player.clazz.color);
			}
		}
		if (outHits != null) {
			int hit = markerSize + 4;
	...
	private static java.util.List<net.minecraft.world.level.saveddata.maps.MapDecoration> collectBlueMarkerDecorations() {
		...
		try {
			...
			// Real bug found (per user report — "the dungeon map still can not remember where everyone is"):
			// BLUE_MARKER alone was an unconfirmed guess (see this method's own commit history) that never
			// actually matched anything — its own diagnostic dump below was even built but never logged, so
			// this was never actually verified one way or the other. Widened to every real vanilla decoration
			// type Hypixel could plausibly be reusing to inject a synthetic "teammate is here" marker into a
			// server-controlled map (the exact same convention vanilla's own client-side "another player is
			// holding this map" tracking uses for its PLAYER icon): PLAYER (the generic rotating arrow every
			// vanilla map already uses for this), plus BLUE_MARKER, PLAYER_OFF_MAP and PLAYER_OFF_LIMITS in
			// case Hypixel's own encoding differs by scenario. Matching more candidate types costs nothing —
			// none of these appear anywhere else on this dungeon map (room/door art uses this codebase's own
			// separate MapScan pixel-color reading, not decorations), so there's no real risk of accidentally
			// grabbing an unrelated icon.
			var markerTypes = java.util.Set.of(
				net.minecraft.world.level.saveddata.maps.MapDecorationTypes.PLAYER,
				net.minecraft.world.level.saveddata.maps.MapDecorationTypes.BLUE_MARKER,
				net.minecraft.world.level.saveddata.maps.MapDecorationTypes.PLAYER_OFF_MAP,
				net.minecraft.world.level.saveddata.maps.MapDecorationTypes.PLAYER_OFF_LIMITS);
			for (var deco : mapData.getDecorations()) {
				if (markerTypes.contains(deco.type())) markers.add(deco);
			}
			// This diagnostic used to build a full raw dump of every decoration's real type/position but never
			// actually logged it anywhere — a dead no-op that made the earlier "not confirmed live" caveat
			// permanently unverifiable. Now actually logged (once per dungeon, not spammed every render
			// frame) so a real report can include the exact type ids Hypixel is sending if this widened set
			// still comes up empty.
			if (markers.isEmpty() && !loggedEmptyMarkerDump) {
				loggedEmptyMarkerDump = true;
				StringBuilder sb = new StringBuilder("DungeonMap: no player-marker decorations found among known types, raw dump:");
				for (var deco : mapData.getDecorations()) {
				...
				com.cokelord.skyblocksimplified.SkyblockSimplified.LOGGER.info(sb.toString());
			} else if (!markers.isEmpty()) {
				loggedEmptyMarkerDump = false;
			}
		} catch (Exception e) {
	...
	/** {@link #headSizeMultiplier}, defaulted to 1.0 for a null instance (matches every other {@code
	 *  instance != null ? instance.field : default} guard already used throughout this class's static
	 *  render helpers, e.g. {@link #showClassSuffix}'s own use in drawTeammateMarker). */
	private static float headSizeMultiplier(DungeonMapFeature inst) {
		return inst != null ? inst.headSizeMultiplier : 1.0f;
	}

	/** The 2px border color {@link #drawRotatedHead} should use for this teammate's head marker: their real
	 *  per-class {@link DungeonClass#color} while {@link #headOutlineClassColor} is on and their class is
	 *  actually known, otherwise the original fixed black. */
	private static int headOutlineColor(DungeonPlayer player) {
		if (instance != null && instance.headOutlineClassColor && player.clazz != null && player.clazz != DungeonClass.EMPTY) {
			return 0xFF000000 | (player.clazz.color & 0xFFFFFF);
		}
		return 0xFF000000;
	}
	...
	private static void drawPlayerMarker(GuiGraphicsExtractor graphics, net.minecraft.world.entity.player.PlayerSkin skin, float px, float py,
										  int markerSize, boolean heads, float yawDegrees, int outlineColor) {
		...
		if (heads && skin != null) {
			try {
				drawRotatedHead(graphics, skin, px, py, markerSize, fx, fz, outlineColor);
				return;
	...
	private static void drawRotatedHead(GuiGraphicsExtractor graphics, net.minecraft.world.entity.player.PlayerSkin skin, float px, float py,
										 int markerSize, float fx, float fz, int outlineColor) {
		...
		try {
			...
			graphics.fill(-half - 2, -half - 2, faceMaxHalf + 2, faceMaxHalf + 2, outlineColor);
	...
	/** Two-line variant of {@link #drawCenteredScaledText} for {@link RoomLabelMode#NAME_AND_SECRETS} — the
	 *  room name on top, secret count below, both centered as one block and scaled down together by
	 *  whichever line is wider (so the name and the count never end up at two different sizes). */
	private static void drawStackedCenteredText(GuiGraphicsExtractor graphics, Minecraft mc, String top, String bottom, float cx, float cy, float cell, int color) {
		int topWidth = mc.font.width(top);
		int bottomWidth = mc.font.width(bottom);
		float maxWidth = cell * 0.9f;
		int widestRaw = Math.max(topWidth, bottomWidth);
		float textScale = widestRaw <= 0 ? 1f : Math.min(1f, maxWidth / widestRaw);
		textScale = Math.max(0.4f, textScale);
		graphics.pose().pushMatrix();
		graphics.pose().translate(cx, cy);
		graphics.pose().scale(textScale);
		graphics.text(mc.font, top, Math.round(-topWidth / 2f), -9, color);
		graphics.text(mc.font, bottom, Math.round(-bottomWidth / 2f), 1, color);
		graphics.pose().popMatrix();
	}
	...
	private static String roomLabel(DungeonRoom room, RoomLabelMode mode) {
		return switch (mode) {
			...
			case NAME, NAME_AND_SECRETS -> room.getName() != null ? room.getName() : "?";
	...
	public float getHeadSizeMultiplier() { return headSizeMultiplier; }
	public void setHeadSizeMultiplier(float value) { headSizeMultiplier = Math.max(0.5f, Math.min(2.5f, value)); }
	public boolean isHeadOutlineClassColor() { return headOutlineClassColor; }
	public void setHeadOutlineClassColor(boolean value) { headOutlineClassColor = value; }
	...
	public boolean isShowClassSuffix() { return showClassSuffix; }
	public void setShowClassSuffix(boolean value) { showClassSuffix = value; }

	private static String classAbbreviation(DungeonClass clazz) {
	...
	public JsonElement savePersistedData() {
		...
		obj.addProperty("headSizeMultiplier", headSizeMultiplier);
		obj.addProperty("headOutlineClassColor", headOutlineClassColor);
		obj.addProperty("revealUnexploredRooms", revealUnexploredRooms);
		obj.addProperty("showNamesMode", showNamesMode.name());
		obj.addProperty("showClassSuffix", showClassSuffix);
	...
	public void loadPersistedData(JsonElement el) {
		...
		if (obj.has("headSizeMultiplier")) headSizeMultiplier = obj.get("headSizeMultiplier").getAsFloat();
		if (obj.has("headOutlineClassColor")) headOutlineClassColor = obj.get("headOutlineClassColor").getAsBoolean();
		...
		if (obj.has("showClassSuffix")) showClassSuffix = obj.get("showClassSuffix").getAsBoolean();
