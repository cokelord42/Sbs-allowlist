...
public class WeirdosSolverFeature extends Feature {
	...
	private void onChatMessage(String text) {
		...
		// Real bug found (per user report — "it only shows one wrong chest and doesnt show the right
		// answer and makes me guess a 50/50"): the 18 CORRECT_PATTERNS/WRONG_PATTERNS entries are meant to
		// cover every real line Hypixel can send (6+12=18, matching the puzzle's own known combinations), so
		// two NPCs going unclassified in the same room means two of their real lines failed to match at all
		// — and matching was done with .matches() (a strict WHOLE-STRING match), the exact same "zero
		// tolerance for anything unexpected" fragility already found and fixed for this class's own
		// NPC_LINE_PATTERN above (see class doc comment) and for other chat lines throughout this codebase.
		// NPC_LINE_PATTERN's own group 2 is greedy ((.+)) with only a trailing OPTIONAL any-char after it, so
		// it can include stray trailing punctuation/whitespace in msg that a strict .matches() against a
		// pattern without that same trailing slack would reject outright. Trimmed and switched to .find()
		// (tolerant of extra leading/trailing content) for both lists, the same fix already applied to this
		// class's own outer line pattern.
		String msg = lineMatch.group(2).trim();

		boolean isCorrect = CORRECT_PATTERNS.stream().anyMatch(p -> p.matcher(msg).find());
		boolean isWrong = !isCorrect && WRONG_PATTERNS.stream().anyMatch(p -> p.matcher(msg).find());
