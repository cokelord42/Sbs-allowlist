...
public class PlayerDisplayFeature extends Feature {
	...
	public final class StatWidget implements MoveableWidget {
		...
		// Per user request ("All the gui elements in the edit gui panel should give an example of their
		// uses"): a fixed representative value per stat, used by renderExample() below instead of the real
		// live field read valueSupplier does.
		...
		private StatWidget(String id, String displayName, String color, String icon, float anchorX, float anchorY,
							String exampleValue, java.util.function.Supplier<String> valueSupplier,
		...
		public Size renderExample(GuiGraphicsExtractor graphics, int x, int y, float scale) {
			return renderText(graphics, x, y, scale, color + exampleValue + (showIcons ? " " + icon : ""));
		}
		...
		private Size renderText(GuiGraphicsExtractor graphics, int x, int y, float scale, String text) {
			...
			// A previous round added a right-align flip here (mirroring TabWidgetOverlayFeature/
			// EstimatedItemValueFeature's own pivot-around-drawX pattern) to fix the box/content mismatch
			// when dragged past screen center. That flip was never mirrored in HudEditScreen's own box
			// computation, though — StatWidget doesn't override rightAlignsPastCenter() (stays false, the
			// MoveableWidget default), so the edit screen's outline box always assumed left-aligned while
			// this render() flipped anyway once past center, which is exactly what put the rendered content
			// outside its own edit-screen box. Removed per user request rather than wiring up the override —
			// always draws left-to-right from the anchor now, consistent with what the edit box assumes.
			int drawX = x;
	...
	public final StatWidget healthWidget = new StatWidget("health", "Player Display: Health", "§c", "❤", 0.32f, 0.85f, "20/20",
		() -> health != null && maxHealth != null ? health + "/" + maxHealth : null, () -> showHealth);
	public final StatWidget defenseWidget = new StatWidget("defense", "Player Display: Defense", "§a", "❈", 0.44f, 0.85f, "150",
		() -> defense != null ? String.valueOf(defense) : null, () -> showDefense);
	public final StatWidget manaWidget = new StatWidget("mana", "Player Display: Mana", "§b", "✎", 0.54f, 0.85f, "500/500",
		() -> mana != null && maxMana != null ? mana + "/" + maxMana : null, () -> showMana);
	public final StatWidget overflowManaWidget = new StatWidget("overflow_mana", "Player Display: Overflow Mana", "§3", "ʬ", 0.50f, 0.81f, "250",
		() -> overflowMana != null && overflowMana > 0 ? String.valueOf(overflowMana) : null, () -> showOverflowMana);
	public final StatWidget vitalityWidget = new StatWidget("vitality", "Player Display: Vitality", "§d", "✛", 0.62f, 0.85f, "150/150",
	...
	public final StatWidget skillXpWidget = new StatWidget("skill_xp", "Player Display: Skill XP", "", "", 0.5f, 0.78f, "§a+13 Combat (24.5%)",
		() -> (skillXpText != null && System.currentTimeMillis() - skillXpUpdatedAtMillis < SKILL_XP_DISPLAY_MILLIS) ? skillXpText : null,
		() -> showSkillXp);
	...
	public final class SpeedWidget implements MoveableWidget {
		...
		@Override
		public Size renderExample(GuiGraphicsExtractor graphics, int x, int y, float scale) {
			return renderText(graphics, x, y, scale, "100%");
		}
