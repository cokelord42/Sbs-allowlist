...
public class TerminalSolverFeature extends Feature {
	...
	private void renderRubix(net.minecraft.client.gui.GuiGraphicsExtractor graphics, AbstractContainerScreen<?> screen,
		...
		for (TerminalTracker.RubixClick click : TerminalTracker.getRubixClicks()) {
			...
			// Real bug found (per user report — "make the rubiks terminals clicks numbers bigger and center
			// them in the click buttons"): this used to draw at a fixed 1x scale tucked into the
			// bottom-right corner — barely legible, and in the wrong spot besides. Scaled to fill most of the
			// slot (bounded by both its height and its width so a 2-digit "-10" never overflows) and drawn
			// centered on the slot's own real center — including in "panel" mode, where `r.size()` can be far
			// bigger than a real 16px inventory slot.
			int textWidth = mc.font.width(countText);
			float textScale = Math.max(1f, Math.min((r.size() * 0.55f) / mc.font.lineHeight,
				(r.size() * 0.8f) / Math.max(1, textWidth)));
			int centerX = r.x() + r.size() / 2;
			int centerY = r.y() + r.size() / 2;
			if (Math.abs(textScale - 1f) < 0.01f) {
				graphics.text(mc.font, countText, centerX - textWidth / 2, centerY - mc.font.lineHeight / 2, 0xFFFFFFFF);
			} else {
				graphics.pose().pushMatrix();
				graphics.pose().translate(centerX, centerY);
				graphics.pose().scale(textScale);
				graphics.pose().translate(-textWidth / 2f, -mc.font.lineHeight / 2f);
				graphics.text(mc.font, countText, 0, 0, 0xFFFFFFFF);
