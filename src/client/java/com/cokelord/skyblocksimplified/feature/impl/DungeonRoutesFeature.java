	...
	protected void onEnable() {
		if (!listenersRegistered) {
			...
			WorldScan.addRoomEnterListener(room -> {
				...
				// Diagnostic logging (per user report — "Enderpearl waypoints doesnt actually show anything" —
				// no functional bug was found after multiple static-review passes over the capture/render
				// pipeline; this prints every positional item's stored relative coords and the resulting real
				// world position once per room entry, so a future report can carry real data instead of
				// another blind guess).
				if (instance.activeRoomName != null && room != null) {
					for (RouteItem it : instance.activeItems()) {
						if (!isPositional(it.type)) continue;
						com.cokelord.skyblocksimplified.SkyblockSimplified.LOGGER.info(
							"[DungeonRoutes] entered {} step#{} type={} relative=({},{},{}) real={}",
							instance.activeRoomName, it.stepNumber, it.type, it.x, it.y, it.z, realPositionOf(room, it));
					}
				}
			});
