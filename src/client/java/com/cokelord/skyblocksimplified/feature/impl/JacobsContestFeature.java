...
public class JacobsContestFeature extends TabWidgetOverlayFeature {
	...
	public Size render(GuiGraphicsExtractor graphics, int x, int y, float scale) {
		...
		JacobContestApi.ContestSlot active = JacobContestApi.getActiveContest();
		if (active != null) {
			long msLeft = active.endEpochMillis() - com.cokelord.skyblocksimplified.gui.HudEditScreen.previewNowMillis();
		...
		long msUntil = next.startEpochMillis() - com.cokelord.skyblocksimplified.gui.HudEditScreen.previewNowMillis();
