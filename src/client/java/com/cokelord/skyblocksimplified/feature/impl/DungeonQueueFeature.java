...
public class DungeonQueueFeature extends Feature implements MoveableWidget {
	...
	// single OTHER member leaving while the party otherwise still exists, which is exactly what "disable on
	...
	// the on-screen warp-cooldown badge used to be tied to nothing but this feature's own enabled state —
	// there was no way to keep Auto Requeue/kick-announce running while hiding just the countdown HUD
	// element. Default true so existing users see no change until they actually flip it off.
		...
		if (!chatRegistered) {
			...
			HudElementRegistry.attachElementBefore(net.fabricmc.fabric.api.client.rendering.v1.hud.VanillaHudElements.PLAYER_LIST, 
				...
				(graphics, deltaTracker) -> {
					if (instance == null || !instance.isVisible() || !com.cokelord.skyblocksimplified.util.IslandGate.isOnHypixel()
	...
	public void setShowTimer(boolean value) { showTimer = value; }
	...
	@Override
	public boolean isVisible() { return isEnabled() && showTimer; }
	...
	public void loadPersistedData(JsonElement data) {
		...
		if (obj.has("showTimer")) showTimer = obj.get("showTimer").getAsBoolean();
