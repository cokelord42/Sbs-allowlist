...
public class LividFinderFeature extends MobHighlightFeature {
	...
	// Real bug found (per user report — "Color livids by livid color doesnt work on the fill"): the
	// wool-detected override above (see onTick's own doc comment) only ever gets threaded through
	// MobHighlightRegistry.setRule's own `color` parameter, which HighlightBoxRenderer only ever applies to
	// the OUTLINE — every fill draw call (2D FULL, 3D FULL_3D) reads feature.getFillColor() directly instead,
	// completely bypassing the per-rule override color. Overriding this accessor is the one place that reaches
	// every one of those fill draw calls at once, without needing to touch HighlightBoxRenderer's own shared
	// (used by every other MobHighlightFeature subclass) fill-color resolution.
	@Override
	public int getFillColor() {
		return colorByLividColor && targetColor != null ? targetColor : super.getFillColor();
	}
