...
public class SlotLockingFeature extends Feature {
	...
	// Per user request ("Allow users to change the color of the locks with the item locker module"): the
	// padlock badge color was a fixed constant with no settings-panel hook at all — same
	// getter/setter/persistence shape every other user-color field in this codebase already follows.
	private int lockColor = 0xFFFFD700;
	 ...
	 *  EquipmentDisplayFeature draws its own duplicate padlock badge (real inventory slots aren't open when
	 *  it renders, so it can't reuse {@link #renderLockIcon} directly) with a hardcoded color literal that
	 *  predates {@link #lockColor} existing as a real setting at all — it never read this field, so
	 *  changing the color here had no visible effect there. Exposed statically so that (and any other future
	 *  "draws its own lock badge") call site stays in sync with the real user setting instead of hardcoding
	 *  a stale copy of the old default. */
	public static int getSharedLockColor() {
		return instance != null ? instance.lockColor : 0xFFFFD700;
	}
	...
	private void renderLockIcon(GuiGraphicsExtractor graphics, int slotScreenX, int slotScreenY) {
		...
		RenderUtil.fillRoundedRing(graphics, x0 + 1, y0 + 1, x0 + 6, y0 + 6, 2, 1, lockColor);
		RenderUtil.fillRounded(graphics, x0 + 1, y0 + 3, x0 + 6, y0 + 7, 1, lockColor);
	}

	public int getLockColor() { return lockColor; }
	public void setLockColor(int value) { lockColor = value; }
	...
	public void loadPersistedData(JsonElement data) {
		...
		if (obj.has("lockColor")) lockColor = obj.get("lockColor").getAsInt();
	}
