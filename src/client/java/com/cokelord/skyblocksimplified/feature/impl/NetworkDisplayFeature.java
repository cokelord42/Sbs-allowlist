...
public class NetworkDisplayFeature extends Feature {
	...
	public final class PingWidget implements MoveableWidget {
		...
		public Size renderExample(GuiGraphicsExtractor graphics, int x, int y, float scale) {
			return renderText(graphics, x, y, scale, "Ping: 42ms", pingColor);
		}
	...
	public final class TpsWidget implements MoveableWidget {
		...
		@Override
		public Size renderExample(GuiGraphicsExtractor graphics, int x, int y, float scale) {
			return renderText(graphics, x, y, scale, "TPS: 20.0", tpsColor);
		}

		@Override
		public Size render(GuiGraphicsExtractor graphics, int x, int y, float scale) {
			double tps = TpsMonitor.getEstimatedTps();
			int color = tpsColorCoded ? tpsColorFor(tps) : tpsColor;
			return renderText(graphics, x, y, scale, "TPS: " + String.format(Locale.ROOT, "%.1f", tps), color);
		}
	}
