...
public class DungeonNotificationsFeature extends Feature implements MoveableWidget {
	...
	public void onTick(Minecraft client) {
		...
		if (!inDungeon) {
			if (inDungeonLastTick) { fired300 = false; fired270 = false; fired5Crypts = false; }
	...
	private void onChatMessage(String text) {
		...
		if (isTypeEnabled(NotificationType.HEALER_SHOULD_ULT) && MAXOR_TRICKED_PATTERN.matcher(text).find()) {
			Minecraft mc = Minecraft.getInstance();
	...
	// a real-looking sample notification instead of just the module's own name.
	private static final String EXAMPLE_TITLE = "§a5 Crypts!";

	@Override
	public Size renderExample(GuiGraphicsExtractor graphics, int x, int y, float scale) {
		return renderText(graphics, x, y, scale, EXAMPLE_TITLE);
	}
