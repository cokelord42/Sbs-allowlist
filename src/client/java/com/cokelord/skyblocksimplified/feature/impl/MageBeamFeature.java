...
public class MageBeamFeature extends Feature {
	...
	// Real bug found (per user report — "the mage beam just doesnt render sometimes"): this used to only
	// ever check the SINGLE most-recently-CREATED beam (activeBeams.get(size()-1)) against a new particle,
	// regardless of how many other beams were also still actively accumulating. MAX_ACTIVE_BEAMS already
	// supports up to 8 concurrent beams (a full dungeon party of Mages can genuinely have several casting
	// within the same couple-tick window) — but the moment a second beam started, every later particle got
	// checked only against THAT new one; the first beam's own list simply stopped receiving particles and
	// went stale (often well short of minPoints), even though its own particles kept arriving in the real
	// stream. Now matches each new particle against whichever ALREADY-ACTIVE beam it's actually closest to
	// (still gated by isInBeamDirection and the same 2-tick recency window) instead of assuming there's
	// only ever one beam "in progress" at a time.
	private void onFireworkParticle(Vec3 point) {
		...
		for (Beam beam : activeBeams) {
			...
			// making it more likely consecutive particles straddle a tick boundary by chance. Widened to
			// tolerate a couple ticks of drift between particles of the same real beam.
			if (currentTick - beam.lastUpdateTick > 2 || !isInBeamDirection(beam.points, point)) continue;
			double distSqr = beam.points.get(beam.points.size() - 1).distanceToSqr(point);
