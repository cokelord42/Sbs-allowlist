...
public class DungeonTimersFeature extends Feature implements MoveableWidget {
	...
	private String purplePadLine() {
		...
		long elapsed = lagAdjustedElapsed(System.currentTimeMillis() - stormPhaseStartMillis) - PURPLE_PAD_DELAY_MILLIS;
	...
	private String goldorStartLine() {
		...
		long elapsed = lagAdjustedElapsed(System.currentTimeMillis() - seenAt);
	...
	// Per user request ("All the gui elements in the edit gui panel should give an example of their uses"):
	// static representative lines, one per real timer this panel can show — no live DungeonState/SplitsFeature
	// reads at all, unlike activeLines() (see renderExample below and MoveableWidget's own doc comment for why
	// that also matters for the edit screen's frame rate).
	private static final List<String> EXAMPLE_LINES = List.of(
		"§dPurple pad: 3.2s", "§6Terminals: 0:42", "§7Goldor Start: §a4.1s", "§bEarly enter: 1.4s",
		"§4Drop in: 4.8s", "§dMaxor moves in: 6.3s");

	@Override
	public Size renderExample(GuiGraphicsExtractor graphics, int x, int y, float scale) {
		return renderLines(graphics, x, y, scale, EXAMPLE_LINES);
	}
