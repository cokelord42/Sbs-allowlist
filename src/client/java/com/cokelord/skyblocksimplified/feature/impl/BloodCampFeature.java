 ...
 * lists Odin uses, via this codebase's existing {@link SkullTextureUtil}. Odin's "Watcher Bar" (boss-bar
 * text amount overlay) sub-feature is dropped — this codebase has no boss-bar text interception hook.
 */
public class BloodCampFeature extends Feature {
	...
	private void updateTracking(ArmorStand entity) {
		...
		// velocity, computed below) times however many ms getTimeUntilSpawn() says are left, added to the
		// head's CURRENT live position — recomputed fresh every render frame in renderInner(), so the box
		// continuously tracks both the head's real position and the countdown's own remaining time.
		long timeTook = Math.max(1, currentTickTime - data.started);
		Vec3 speedVectors = new Vec3(
			(currentPos.x - data.startVector.x) / timeTook,
			(currentPos.y - data.startVector.y) / timeTook,
			(currentPos.z - data.startVector.z) / timeTook);

		RenderData render = renderDataMap.computeIfAbsent(entity, e -> new RenderData());
		render.speedVectors = speedVectors;
	...
	private void renderInner(GuiGraphicsExtractor graphics) {
		...
		for (var entry : renderDataMap.entrySet()) {
			...
			// See updateTracking's own doc comment ("detect the movement speed of the head and do the math
			// on where it will be when the timer is finished"): recomputed fresh every frame directly from
			// the head's real current position plus its own observed average velocity times however many ms
			// are left on the countdown — inherently smooth frame to frame (both the entity's own position
			// and the remaining time change gradually), so no separate interpolation step is needed the way
			// the old fixed-distance/direction endpoint required.
