...
public class BlazeSolverFeature extends Feature {
	...
	public void onTick(Minecraft client) {
		...
		// Real bug found (per user report — "The blazes don't reset when I architect draft"): a wrong-order
		// kill FAILS the puzzle, not completes it, but this class has no separate "failed" signal at all —
		// the only outcome it ever records is COMPLETED, set below whenever the tracked count reaches exactly
		// 1 and then drops to 0. A failure that happens to occur with exactly one blaze left (a very common
		// case — the LAST kill is usually the one that goes wrong) hits that exact same "1 -> 0" transition
		// and gets mis-recorded as a real completion. Using the Architect's Draft item (see
		// AutoArchitectDraftFeature's own doc comment on the real fetch-a-retry mechanic this fixes for) then
		// spawns a brand new set of blaze armor stands for a fresh attempt — but Puzzle.BLAZE.status stayed
		// stuck at the stale COMPLETED from the failed attempt forever, since nothing ever told it a new
		// attempt had started. Real blazes reappearing after a COMPLETED status was already recorded is
		// exactly that signal (a genuinely solved puzzle's blazes don't come back) — clears it back to
		// "not yet decided" the moment that happens, so the fresh attempt gets a real completion check again.
		if (!nearby.isEmpty() && Puzzle.BLAZE.status == PuzzleStatus.COMPLETED) {
			Puzzle.BLAZE.status = null;
		}
