...
public final class IslandGate {
	 ...
	 *
	 *  <p>Per user request ("Replace Hypixel Mod API dependency module with an always-on warning banner"):
	 *  no longer gated on a user-facing toggle — the companion mod is strictly more reliable than the
	 *  sidebar-scan fallback (see this class's own doc comment), so there was never a real reason to let the
	 *  user opt out of it while it's installed. HypixelModApiDependencyFeature now just surfaces an
	 *  informational status row in the GUI instead of a switch. */
