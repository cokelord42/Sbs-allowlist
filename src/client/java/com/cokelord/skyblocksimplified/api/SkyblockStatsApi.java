 ...
 *
 * <p>Deliberately does NOT compute a "Catacombs Level" number from {@code catacombs.experience} — that
 * requires the real per-level dungeon XP table, which is a separate, non-trivial constant this codebase has
 * no verified source for (not present in Hypixel's own API/resources, and not confidently recoverable from
 * memory without risking a silently-wrong number for exactly the kind of "real value" this project's own
 * established policy says never to guess). The raw XP is exposed instead so callers can show it honestly
	...
	private static PlayerStats parse(JsonObject root, String uuid) {
		...
		if (petsData.has("pets") && petsData.get("pets").isJsonArray()) {
			for (JsonElement pe : petsData.getAsJsonArray("pets")) {
				...
				if (pet.has("type") && pet.get("type").isJsonPrimitive() && GOLDEN_DRAGON_TYPE.equals(pet.get("type").getAsString())) {
					hasGoldenDragon = true;
					break;
				}
			}
		}
		...
		return new PlayerStats(f7, m5, m6, m7, totalSecrets, totalRuns, catacombsXp,
			hasHyperion[0], hasTerminator[0], hasGoldenDragon, hasCatacombsExpertRing[0], bankBalance, armorNames, equipmentNames,
